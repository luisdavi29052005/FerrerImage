    ...
    const handlePaymentSuccess = useCallback(async () => {
        ...
        if (paymentRequest.type === 'single' && paymentRequest.decade) {
            ...
            if (image?.status === 'done' && image.url) {
                ...
                link.href = image.url;
                link.download = `image-ferrer-${decade}.jpg`;
            ...
            try {
                // FIX: Explicitly type the `image` parameter as `GeneratedImage` to resolve TypeScript inference issues in both `filter` and `reduce`.
                ...
                if (Object.keys(imageData).length === 0) {
                    alert("No images have been generated successfully to create an album.");
                    return;
                }
                ...
                link.download = 'image-ferrer-album.jpg';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
